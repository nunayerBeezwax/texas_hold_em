...
before do
	...
	describe '#initialize' do
		it 'makes and shuffles a deck from which to deal random cards' do
			test_game = Game.new
			test_game.dealer.deck.deck.length.should eq 52
			test_game.dealer.deck.deck[33].should be_an_instance_of Card
		end
	end
	...
	describe '#preflop' do
		it 'removes cards from the deck, gameing 2 cards to each player' do
			...
			test_game.table.table[5].hole_cards.length.should eq 2
			test_game.table.table[8].hole_cards.length.should eq 2
			test_game.table.table[0].hole_cards.length.should eq 2
			test_game.dealer.deck.deck.length.should eq 34
		end
	end
	...
	describe '#flop' do
		it 'removes 3 cards from the deck and displays them as community cards' do
			...
			test_game.dealer.deck.deck.length.should eq 31
		end
	end
